...
class PosOrder(models.Model):
    ...
    def _saga_validate_exportable(self, anonymous_client_name="CLIENT DIVERS", include_receipts=True):
        ...
        missing_company_vat = self.filtered(lambda order: not order.company_id.partner_id.vat)
        if missing_company_vat:
            raise UserError(_("Your company needs a VAT/CIF value before exporting POS orders to Saga XML."))
    ...
    def _saga_build_invoice_element(self, date_format="romanian", anonymous_client_name="CLIENT DIVERS"):
        ...
        supplier_values = self._saga_prepare_party_values(self.company_id.partner_id.commercial_partner_id)
    ...
    def _saga_invoice_filename(self):
        ...
        return f"F_{company_vat}_{number}_{order_date.isoformat()}.xml"

    def _saga_receipts_filename(self, payment_date):
        return f"I_{payment_date.isoformat()}.xml"

    def _saga_zip_filename(self):
        company_vat = self._saga_filename_part(self._saga_normalize_vat(self[:1].company_id.partner_id.vat))
