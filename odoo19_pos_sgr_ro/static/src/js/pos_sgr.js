...
function getAllProducts(pos) {
    ...
    if (byId && typeof byId === "object") {
        for (const product of Object.values(byId)) {
    ...
    const modelProducts = pos?.models?.["product.product"];
    if (modelProducts) {
        const iterable = Array.isArray(modelProducts)
            ? modelProducts
            : typeof modelProducts.getAll === "function"
...
patch(Order.prototype, {
    ...
    _getSgrDepositProduct() {
        return getAllProducts(this.pos).find((product) => isTruthy(product?.sgr_is_deposit_product)) || null;
    },
    ...
    _syncSgrForOrderline(orderline) {
        ...
        try {
            ...
            const qty = Number(orderline.get_quantity());
            if (!qty || qty <= 0) {
                if (existingSgr) {
                    super.removeOrderline(existingSgr);
                }
